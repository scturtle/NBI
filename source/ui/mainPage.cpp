...
void mathstuff() {
  ...
  if (inst::ui::maini_theme && inst::config::useTheme &&
      std::filesystem::exists(inst::config::appDir + "/theme/theme.json") &&
      std::filesystem::exists(inst::config::appDir + "icons_others.drive"_theme)) {
...
void playmusic() {
  ...
  if (inst::ui::maini_theme && inst::config::useTheme &&
      std::filesystem::exists(inst::config::appDir + "/theme/theme.json") &&
      std::filesystem::exists(inst::config::appDir + "audio.music"_theme)) {
...
MainPage::MainPage() : Layout::Layout() {
  ...
  if (inst::ui::maini_theme && inst::config::useTheme &&
      std::filesystem::exists(inst::config::appDir + "/theme/theme.json") && std::filesystem::exists(main_top))
    this->titleImage = Image::New(0, 0, pu::sdl2::TextureHandle::New(pu::ui::render::LoadImage((main_top))));
  else
    this->titleImage =
  ...
  if (inst::ui::maini_theme && inst::config::useTheme &&
      std::filesystem::exists(inst::config::appDir + "/theme/theme.json"))
    this->optionMenu =
        pu::ui::elm::Menu::New(0, 95, 1280, COLOR(background_overlay1), COLOR(background_overlay2), 94, 6);
  else
    this->optionMenu = pu::ui::elm::Menu::New(0, 95, 1280, COLOR("#FFFFFF00"), COLOR("#4f4f4d33"), 94, 6);

  if (inst::ui::maini_theme && inst::config::useTheme &&
      std::filesystem::exists(inst::config::appDir + "/theme/theme.json"))
    this->optionMenu->SetItemsFocusColor(COLOR(focus));
  else
    this->optionMenu->SetItemsFocusColor(COLOR("#4f4f4dAA"));

  if (inst::ui::maini_theme && inst::config::useTheme &&
      std::filesystem::exists(inst::config::appDir + "/theme/theme.json"))
    this->optionMenu->SetScrollbarColor(COLOR(scrollbar));
  else
  ...
  if (inst::ui::maini_theme && inst::config::useTheme &&
      std::filesystem::exists(inst::config::appDir + "/theme/theme.json"))
    this->installMenuItem->SetColor(COLOR(text_colour));
  else
    this->installMenuItem->SetColor(COLOR("#FFFFFFFF"));
  if (inst::ui::maini_theme && inst::config::useTheme &&
      std::filesystem::exists(inst::config::appDir + "/theme/theme.json") && std::filesystem::exists(icons_sd))
    this->installMenuItem->SetIcon(pu::sdl2::TextureHandle::New(pu::ui::render::LoadImage(icons_sd)));
  else
  ...
  if (inst::ui::maini_theme && inst::config::useTheme &&
      std::filesystem::exists(inst::config::appDir + "/theme/theme.json"))
    this->netInstallMenuItem->SetColor(COLOR(text_colour));
  else
    this->netInstallMenuItem->SetColor(COLOR("#FFFFFFFF"));
  if (inst::ui::maini_theme && inst::config::useTheme &&
      std::filesystem::exists(inst::config::appDir + "/theme/theme.json") && std::filesystem::exists(icons_net))
    this->netInstallMenuItem->SetIcon(pu::sdl2::TextureHandle::New(pu::ui::render::LoadImage(icons_net)));
  else
    this->netInstallMenuItem->SetIcon(
        pu::sdl2::TextureHandle::New(pu::ui::render::LoadImage("romfs:/images/icons/cloud-download.png")));

  this->usbInstallMenuItem = pu::ui::elm::MenuItem::New("main.menu.usb"_lang);
  if (inst::ui::maini_theme && inst::config::useTheme &&
      std::filesystem::exists(inst::config::appDir + "/theme/theme.json"))
    this->usbInstallMenuItem->SetColor(COLOR(text_colour));
  else
    this->usbInstallMenuItem->SetColor(COLOR("#FFFFFFFF"));
  if (inst::ui::maini_theme && inst::config::useTheme &&
      std::filesystem::exists(inst::config::appDir + "/theme/theme.json") && std::filesystem::exists(icons_usb))
    this->usbInstallMenuItem->SetIcon(pu::sdl2::TextureHandle::New(pu::ui::render::LoadImage(icons_usb)));
  else
    this->usbInstallMenuItem->SetIcon(
        pu::sdl2::TextureHandle::New(pu::ui::render::LoadImage("romfs:/images/icons/usb-port.png")));

  this->HdInstallMenuItem = pu::ui::elm::MenuItem::New("main.menu.hdd"_lang);
  if (inst::ui::maini_theme && inst::config::useTheme &&
      std::filesystem::exists(inst::config::appDir + "/theme/theme.json"))
    this->HdInstallMenuItem->SetColor(COLOR(text_colour));
  else
    this->HdInstallMenuItem->SetColor(COLOR("#FFFFFFFF"));
  if (inst::ui::maini_theme && inst::config::useTheme &&
      std::filesystem::exists(inst::config::appDir + "/theme/theme.json") && std::filesystem::exists(icons_hdd))
    this->HdInstallMenuItem->SetIcon(pu::sdl2::TextureHandle::New(pu::ui::render::LoadImage(icons_hdd)));
  else
  ...
  if (inst::ui::maini_theme && inst::config::useTheme &&
      std::filesystem::exists(inst::config::appDir + "/theme/theme.json"))
    this->settingsMenuItem->SetColor(COLOR(text_colour));
  else
    this->settingsMenuItem->SetColor(COLOR("#FFFFFFFF"));
  if (inst::ui::maini_theme && inst::config::useTheme &&
      std::filesystem::exists(inst::config::appDir + "/theme/theme.json") && std::filesystem::exists(icons_settings))
    this->settingsMenuItem->SetIcon(pu::sdl2::TextureHandle::New(pu::ui::render::LoadImage(icons_settings)));
  else
  ...
  if (inst::ui::maini_theme && inst::config::useTheme &&
      std::filesystem::exists(inst::config::appDir + "/theme/theme.json"))
    this->exitMenuItem->SetColor(COLOR(text_colour));
  else
    this->exitMenuItem->SetColor(COLOR("#FFFFFFFF"));
  if (inst::ui::maini_theme && inst::config::useTheme &&
      std::filesystem::exists(inst::config::appDir + "/theme/theme.json") && std::filesystem::exists(icons_exit))
    this->exitMenuItem->SetIcon(pu::sdl2::TextureHandle::New(pu::ui::render::LoadImage(icons_exit)));
  else
  ...
  if (nx::hdd::count() && nx::hdd::rootPath()) {
    if (inst::ui::maini_theme && inst::config::useTheme &&
        std::filesystem::exists(inst::config::appDir + "/theme/theme.json") &&
        std::filesystem::exists(icons_hdd_connected))
      this->hdd = Image::New(1156, 669, pu::sdl2::TextureHandle::New(pu::ui::render::LoadImage(icons_hdd_connected)));
    else
      this->hdd = Image::New(
          1156, 669,
          pu::sdl2::TextureHandle::New(pu::ui::render::LoadImage("romfs:/images/icons/usb-hd-connected.png")));
    this->Add(this->hdd);
  }
    ...
    if (inst::ui::maini_theme && inst::config::useTheme &&
      ...
      inst::ui::mainApp->CreateShowDialog("main.net.title"_lang, "main.net.desc"_lang, {"common.ok"_lang}, true,
                                          pu::sdl2::TextureHandle::New(pu::ui::render::LoadImage(
                                              inst::config::appDir + "icons_others.information"_theme)));
    } else
      inst::ui::mainApp->CreateShowDialog(
          "main.net.title"_lang, "main.net.desc"_lang, {"common.ok"_lang}, true,
          pu::sdl2::TextureHandle::New(pu::ui::render::LoadImage("romfs:/images/icons/information.png")));
    return;
  }
...
void MainPage::usbInstallMenuItem_Click() {
  ...
  if (inst::ui::maini_theme && inst::config::useTheme &&
      std::filesystem::exists(inst::config::appDir + "/theme/theme.json") &&
      std::filesystem::exists(inst::config::appDir + "icons_others.usb"_theme)) {
    usb = inst::config::appDir + "icons_others.usb"_theme;
  }
  if (!inst::config::usbAck) {
    if (mainApp->CreateShowDialog("main.usb.warn.title"_lang, "main.usb.warn.desc"_lang,
                                  {"common.ok"_lang, "main.usb.warn.opt1"_lang}, false,
                                  pu::sdl2::TextureHandle::New(pu::ui::render::LoadImage(usb))) == 1) {
  ...
  if (inst::util::getUsbState() == 5)
    ...
    mainApp->CreateShowDialog("main.usb.error.title"_lang, "main.usb.error.desc"_lang, {"common.ok"_lang}, true,
                              pu::sdl2::TextureHandle::New(pu::ui::render::LoadImage(usb)));
}
