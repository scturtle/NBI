...
pu::sdl2::TextureHandle::Ref unchecked_theme_tex =
    pu::sdl2::TextureHandle::New(pu::ui::render::LoadImage(unchecked_theme));

void checkbox_theme() {
  if (istheme && inst::config::useTheme && std::filesystem::exists(inst::config::appDir + "/theme/theme.json") &&
...
ThemeInstPage::ThemeInstPage() : Layout::Layout() {
  ...
  if (istheme && inst::config::useTheme && std::filesystem::exists(inst::config::appDir + "/theme/theme.json") &&
      std::filesystem::exists(theme_top))
    this->titleImage = Image::New(0, 0, pu::sdl2::TextureHandle::New(pu::ui::render::LoadImage((theme_top))));
  else
    this->titleImage =
        Image::New(0, 0, pu::sdl2::TextureHandle::New(pu::ui::render::LoadImage("romfs:/images/Net.png")));
  ...
  if (istheme && inst::config::useTheme && std::filesystem::exists(inst::config::appDir + "/theme/theme.json") &&
    ...
    this->infoImage = Image::New(
        453, 292,
        pu::sdl2::TextureHandle::New(pu::ui::render::LoadImage("romfs:/images/icons/lan-connection-waiting.png")));
  ...
  if (istheme && inst::config::useTheme && std::filesystem::exists(inst::config::appDir + "/theme/theme.json"))
    this->installBar->SetBackgroundColor(COLOR(progress_bg_colour));
  else
    this->installBar->SetBackgroundColor(COLOR("#000000FF"));
  if (istheme && inst::config::useTheme && std::filesystem::exists(inst::config::appDir + "/theme/theme.json"))
    this->installBar->SetProgressColor(COLOR(progress_fg_colour));
  else
...
void ThemeInstPage::selectTitle(int selectedIndex) {
  if (installing != 1) {
    for (long unsigned int i = 0; i < this->selectedUrls.size(); i++) {
      ...
      if (didDownload) {
        ...
        if (inst::config::useSound) {
          ...
          if (istheme && inst::config::useTheme &&
        ...
        inst::ui::mainApp->CreateShowDialog("theme.theme_error"_lang, "theme.theme_error_info"_lang, {"common.ok"_lang},
                                            true, pu::sdl2::TextureHandle::New(pu::ui::render::LoadImage(fail)));
        return;
      }
      ...
      if (didExtract) {
        ...
        if (istheme && inst::config::useTheme && std::filesystem::exists(inst::config::appDir + "/theme/theme.json") &&
            std::filesystem::exists(inst::config::appDir + "icons_others.good"_theme)) {
          good = inst::config::appDir + "icons_others.good"_theme;
        }
        int close = inst::ui::mainApp->CreateShowDialog("theme.installed"_lang, "theme.restart"_lang,
                                                        {"sig.later"_lang, "sig.restart"_lang}, true,
    ...
    inst::ui::mainApp->CreateShowDialog("theme.wait"_lang, "theme.trying"_lang, {"common.ok"_lang}, true,
...
void ThemeInstPage::onInput(u64 Down, u64 Up, u64 Held, pu::ui::TouchPoint touch_pos) {
  ...
  if (Down & HidNpadButton_X) {
    ...
    if (exists == true) {
      ourResult = inst::ui::mainApp->CreateShowDialog("theme.remove"_lang, "theme.delete"_lang,
                                                      {"common.no"_lang, "common.yes"_lang}, true,
                                                      pu::sdl2::TextureHandle::New(pu::ui::render::LoadImage(theme)));
    }

    else {
      inst::ui::mainApp->CreateShowDialog("theme.notfound"_lang, "theme.notfound2"_lang, {"common.ok"_lang}, false,
                                          pu::sdl2::TextureHandle::New(pu::ui::render::LoadImage(theme)));
      return;
    }
    ...
    if (ourResult != 0) {
      try {
        ...
        if (done == true) {
          theme = "romfs:/images/icons/theme.png"; // if the theme was removed the icon will be missing so show this
                                                   // instead.
          inst::ui::mainApp->CreateShowDialog("theme.notice"_lang, "theme.success"_lang, {"common.ok"_lang}, false,
                                              pu::sdl2::TextureHandle::New(pu::ui::render::LoadImage(theme)));
          mainApp->FadeOut();
          mainApp->Close();
        } else {
          inst::ui::mainApp->CreateShowDialog("theme.notice"_lang, "theme.notremoved"_lang, {"common.ok"_lang}, false,
                                              pu::sdl2::TextureHandle::New(pu::ui::render::LoadImage(theme)));
        }
        ...
        inst::ui::mainApp->CreateShowDialog("theme.warning"_lang, "theme.error"_lang, {"common.ok"_lang}, false,
    ...
    if (netConnected2) {
      if ((Down & HidNpadButton_A) || (state.count != xxxx)) {
        ...
        if (xxxx != 1) {
          ...
          if (s != "0") {
            myindex =
                this->menu->GetSelectedIndex(); // store index so when page redraws we can get the last item we checked.
