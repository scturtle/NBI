...
void installSigPatches() {
  ...
  if (inst::ui::sigi_theme && inst::config::useTheme &&
  ...
  if (inst::ui::sigi_theme && inst::config::useTheme &&
      std::filesystem::exists(inst::config::appDir + "/theme/theme.json") &&
      std::filesystem::exists(inst::config::appDir + "icons_others.patches"_theme)) {
  ...
  try {
    ...
    int ourResult =
        inst::ui::mainApp->CreateShowDialog("sig.title0"_lang, "sig.desc0"_lang + versionText,
                                            {installButtonText, "sig.uninstall"_lang, "common.cancel"_lang}, true,
                                            pu::sdl2::TextureHandle::New(pu::ui::render::LoadImage(patches)));
    if (ourResult == 0) {
      ...
      if (!inst::util::copyFile("sdmc:/bootloader/patches.ini", inst::config::appDir + "/patches.ini.old")) {
        if (inst::ui::mainApp->CreateShowDialog("sig.backup_failed"_lang, "sig.backup_failed_desc"_lang,
      ...
      if (didDownload)
        ...
        inst::ui::mainApp->CreateShowDialog("sig.download_failed"_lang, "sig.download_failed_desc"_lang,
      ...
      if (didExtract) {
        ...
        versionText = "";
        if (patchesVersion != "")
          versionText = "sig.version_text2"_lang + patchesVersion + "! ";
        if (inst::ui::mainApp->CreateShowDialog("sig.install_complete"_lang,
                                                versionText + "\n\n" + "sig.complete_desc"_lang,
                                                {"sig.restart"_lang, "sig.later"_lang}, false,
                                                pu::sdl2::TextureHandle::New(pu::ui::render::LoadImage(good))) == 0)
          bpcRebootSystem();
      } else {
      ...
      if (!inst::util::copyFile(inst::config::appDir + "/patches.ini.old", "sdmc:/bootloader/patches.ini")) {
        if (inst::ui::mainApp->CreateShowDialog("sig.restore_failed"_lang, "", {"common.yes"_lang, "common.no"_lang},
                                                false, pu::sdl2::TextureHandle::New(pu::ui::render::LoadImage(fail))))
          return;
      } else
        std::filesystem::remove(inst::config::appDir + "/patches.ini.old");
      if (inst::util::removeDirectory("sdmc:/atmosphere/exefs_patches/es_patches")) {
        if (inst::ui::mainApp->CreateShowDialog("sig.uninstall_complete"_lang, "sig.complete_desc"_lang,
                                                {"sig.restart"_lang, "sig.later"_lang}, false,
                                                pu::sdl2::TextureHandle::New(pu::ui::render::LoadImage(good))) == 0)
          bpcRebootSystem();
      } else
        inst::ui::mainApp->CreateShowDialog("sig.remove_failed"_lang, "sig.remove_failed_desc"_lang, {"common.ok"_lang},
